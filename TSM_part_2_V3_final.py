...
def readfile(file):
    ...
    returns = df['GBPUSD']
    describtive_data(returns)
    
    return returns


def part_c(x, kappa_ini, phi_ini, sigma2_n_ini):
    initial_params = (kappa_ini, phi_ini, sigma2_n_ini)
...
def log_likelihood(y, params):
    n = len(y)
    kappa, phi, sigma2_n = (params)

    # run Kalman filter
    filtered_a, filtered_P, a, P, v, F, K = kalman_filter(y, kappa=kappa, phi=phi, sigma2_n=sigma2_n)
...
def kalman_filter(y, kappa, phi, sigma2_n):   
    # initialize variables
    n = len(y)
    filtered_a, filtered_P = np.empty(n), np.empty(n)
    a, P = np.empty(n+1), np.empty(n+1) 
    v, F = np.empty(n), np.empty(n)
    K = np.empty(n)
   
    a[0] = 0
    P[0] = sigma2_n / (1 - phi**2)
    ...
    for t in range(n-1): # n-1
        ...
        P[t + 1] = phi * filtered_P[t] * phi + sigma2_n # T_t * P_t|t * T_t' + R_t * Q_t * R_t', where T_t = phi, R_t = 1 & Q_t = sigma_n
...
def plot_kalman(date, a, a_hat, x_t, kappa):
    ...
    #axs[0].plot(date, np.exp(a_hat/2), label=r'Smoothed $\alpha_t$') #### <--- test plot in book
...
def mode_estimation(y, kappa, phi, sigma2_eta):
    ...
    convergence_bound = 10 ** (-7)
    y_doublebar = (y - np.mean(y)) / sigma

    iteration = 0

    while True:
        A = 2 * np.exp(g) / (y_doublebar) ** 2
        z = g + 1 - np.exp(g) / (y_doublebar) ** 2
        ...
        if np.all(np.abs(g_plus - g) <= convergence_bound):
            mode = g_plus
            break
        else:
            g = g_plus
...
def main():
    ...
    sigma2_n_ini = (1 - phi_ini**2) * (np.var(x_t, ddof=1) - ((np.pi**2)/2))
    a_1 = 0 #np.random.normal(0, np.sqrt(sigma2_n_ini/(1 - phi_ini**2)))
    P_1 = sigma2_n_ini / (1 - phi_ini**2)

    result = part_c(x_t, kappa_ini, phi_ini, sigma2_n_ini)
    ...
    print("Log-Likelihood value:", np.round(log_likelihood_value, 4))
    
    print("Estimates (kappa, phi, sigma_n):", 
    ...
    sigma_n_MoM = (1 - (result.x[1]**2)) * (np.var(x_t, ddof=1) - ((np.pi**2) / 2))
    
    print("\nMoM estimate of sigma eta:", np.round(sigma_n_MoM, 4), "versus QML estimate:", np.round(result.x[2], 4))
    print("Fraction:", np.round((result.x[2] / sigma_n_MoM), 4))
    
    n = 10000
    x_simulated = simulate_data(n, kappa_ini, phi_ini, sigma2_n_ini)
    simulated_result = part_c(x_simulated, kappa_ini, phi_ini, sigma2_n_ini)
    ...
    # Plot the results
    plt.figure(figsize=(20, 8)) #figsize=(10, 5)
